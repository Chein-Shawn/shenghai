...
- Increased the CPDL batch size to 500. The batch completed successfully and increased the compact manifest to 1,623 records: 159 paired, 93 PDF-only, 8 MusicXML-only, and 1,363 missing. The external SSD now contains 252 PDFs and 198 MusicXML/MXL files; 1,708 paired-link candidates remain unresolved.
- Ran one continuation without a batch limit over the remaining pending paired-link records. It processed 531 candidates and increased the compact manifest to 2,154 records: 249 paired, 109 PDF-only, 14 MusicXML-only, and 1,782 missing. The external SSD now contains 358 PDFs and 302 MusicXML/MXL files; 1,596 paired-link candidates remain unresolved.
- Created external-SSD dataset release `prepared/cpdl-v1`: validated 249 paired records, split by score into 174 train / 37 validation / 38 test, and rasterized 874 pages at 300 DPI. Generated 3,093 heuristic system candidates. All candidates remain `manual_review_required` because MusicXML does not encode the original PDF page/system boundaries.
- Added `finalize_cpdl_systems.py`; it creates MusicXML fragments and LMX token targets only after a reviewer marks a system row `verified` and supplies `measure_start`/`measure_end`. No unverified CPDL data is being used for model training.
...
- Fixed the preview memory key to use the whole score ID rather than the page/system ID. All systems from one multi-page score now share the same saved scroll position and zoom preference.
- Changed Correct and Incorrect navigation to advance sequentially to the next system rather than jumping to the next pending item. Revisiting system 11 and checking it again now leads to system 12, even when system 12 was already reviewed.
- Restricted keyboard navigation to non-editing areas. Left/right arrows typed inside measure fields or review notes now move the text cursor instead of changing the current system.
- Expanded the CPDL reviewer default to the complete `cpdl-v1` manifest. `--limit-scores 0` now presents all 3,093 systems while preserving the existing reviewed rows and notes; `--limit-scores 50` remains available for a smaller session.
- Backed up the reviewer navigation and preview changes to GitHub as commit `3b09154` before starting dataset processing.
...
- Made symbol types runtime-editable through the external-SSD `symbols/symbol-kinds.json` configuration and annotation-page controls. Added/removed types are persisted; removed types do not erase historical boxes, and `other` remains a protected fallback.
...
# 2026-07-13: Staged symbol-OMR pretraining and assisted CPDL annotation

- Fixed the DeepScoresV2-to-VocalDive mapping so `keyFlat`, `keySharp`, and `keyNatural` consistently map to the stable `key_signature` channel. The first product-relevant detector interface keeps fourteen channels, while the first pretraining gate concentrates on ten primary printed-symbol categories: noteheads, rests, barlines, clefs, key signatures, time signatures, accidentals, stems, beams, and dots.
- Reworked the heatmap trainer to report class-wise precision, recall, and IoU at several confidence thresholds. Checkpoint selection now uses held-out primary-class mean IoU rather than BCE loss alone, and the report records the selected checkpoint, epoch, and score.
- Ran the first deterministic DeepScores stage on 1,024 training tiles and 256 held-out tiles for five epochs on MPS. Validation primary mean IoU improved from `0.0649` to `0.0846`; noteheads, beams, and stems showed the first usable signal, while small or infrequent classes remain weak. This is a pretraining signal only, not choir-domain OMR quality and not eligible for Core ML export.
- Ran the planned 8,192-tile expansion against 1,024 held-out tiles. The best completed checkpoint was epoch two at primary mean IoU `0.1190` (epoch one `0.1008`, epoch three `0.1173`). The non-improving third epoch triggered an early stop rather than spending further compute without evidence of benefit. A 150-system CPDL prediction-preview manifest was generated from the best checkpoint; those previews are explicitly non-ground-truth annotation aids.
- The expanded run also showed that the current bottleneck is repeated high-resolution crop/resize work on the external SSD, not MPS memory. The trainer now writes an atomic progress report after every epoch, preserving metrics and the selected checkpoint if a future long run is stopped.
- Expanded the CPDL symbol reviewer queue from 52 to 150 diverse systems while preserving the two existing annotations, fifty prior pending rows, IDs, and notes. New queue entries are selected from previously unseen scores first.
- Added a required `core_annotation_complete` confirmation. A CPDL system cannot enter fine-tuning until every visible primary symbol has been boxed; advanced labels remain stored as metadata and are never silently interpreted as background.
