...
class AnnotationServer:
    ...
    def state(self) -> dict[str, object]:
        ...
        return {"rows": self.rows, "kinds": self.kinds, "total": len(self.rows), "annotated": done, "pending": len(self.rows) - done, "firstPending": first, "index": first}
...
def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--version", type=Path, default=DEFAULT_VERSION); parser.add_argument("--manifest", type=Path); parser.add_argument("--output", type=Path); parser.add_argument("--kinds-config", type=Path); parser.add_argument("--limit", type=int, default=50, help="Pending systems in batch; 0 means all verified systems"); parser.add_argument("--port", type=int, default=8766)
    args = parser.parse_args(); version = args.version.expanduser().resolve(); source = (args.manifest or version / "manifests/system-candidate-manifest.jsonl").expanduser().resolve(); output = (args.output or version / "symbols/symbol-annotation-manifest.jsonl").expanduser().resolve(); kinds_config = (args.kinds_config or version / "symbols/symbol-kinds.json").expanduser().resolve(); state = AnnotationServer(source, output, kinds_config, args.limit); http = ThreadingHTTPServer(("127.0.0.1", args.port), make_handler(state)); print(f"Symbol annotation server: http://127.0.0.1:{args.port}/", flush=True); print(f"Derived manifest: {output}", flush=True); print(f"Type configuration: {kinds_config}", flush=True); http.serve_forever()
